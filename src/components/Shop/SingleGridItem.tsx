  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        <Image 
          src={item.thumbnails?.[0] || item.imgs?.previews?.[0] || item.imgs?.thumbnails?.[0] || "/images/placeholder.jpg"} 
          alt={item.title} 
          width={250} 
          height={250} 
        />
      ...
      <h3 className="font-medium text-dark ease-out duration-200 hover:text-blue mb-1.5">
        <Link href={`/shop-details/${item.slug?.current || item.title?.toLowerCase().replace(/\s+/g, '-')}`}> 
          {item.title} 
        </Link>
      </h3>
