...
const ShopWithSidebar = () => {
  ...
  // Start with an empty list so no demo products show by default
  ...
  const applyFilters = () => {
    ...
    if (selectedCategories.length > 0) {
      filteredProducts = filteredProducts.filter(product => 
        product.category && typeof product.category === 'object' && selectedCategories.includes(product.category._id)
      );
    }
    ...
    const fetchData = async () => {
      try {
        ...
        if (fetchedProducts.length > 0) {
          const prices = fetchedProducts.map(p => p.discountedPrice || p.price);
          const maxPrice = Math.max(...prices);
          setPriceRange({ min: 0, max: maxPrice });
        }
      } catch (error) {
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filters:</p>
                      <button onClick={clearAllFilters} className="text-blue">Clean All</button>
                    </div>
                  </div>
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      Showing <span className="text-dark">{startIndex + 1}-{Math.min(endIndex, products.length)}</span> of{" "}
              ...
              {products.length === 0 ? (
                ...
                >
                  {currentProducts.map((item, key) =>
                    productStyle === "grid" ? (
                      <SingleGridItem item={item} key={item._id || key} />
                    ) : (
                      <SingleListItem item={item} key={item._id || key} />
                    )
                  )}
                </div>
              )}
